...
public class SparkDriver {
    ...
    public static void main(String[] args) {
        ...
        SparkConf conf = new SparkConf().setAppName("Exam20210630 - Spark V2");
        ...
        // Part 2
        // count for each motorbike model the number of sales
        ...
        // Join the countSales RDD (modelID, count) with the transformed 
        // motorbikesRDD (modelID, manufacturer) using a right outer join
        // unsold motorbike models have count == None
        // map unsold motorbike models from count == None to count = 0
        // then, filter only infrequent or unsold motorbike models
        JavaPairRDD<String, Tuple2<Integer, String>> unsoldInfreqModelsManufRDD = countSalesRDD
                                                                    .rightOuterJoin(modelManufacturerRDD)
                                                                    .mapValues(it -> {
                                                                        int count = it._1().orElse(0);
                                                                        return new Tuple2<>(count, it._2());
                                                                    })
                                                                    .filter(it -> it._2()._1() <= 10);

        // Map the previously computed RDD (modelID, (countSales, Manufacturer))
        // into (Manufacturer, +1)
        // and count the number of infrequent + unsold motorbike models for each manufacturer
        // The result is (manufacturer, countUnsoldInfrequentMotorbikeModels)
